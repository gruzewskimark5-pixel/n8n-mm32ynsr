...
## 2026-04-06 - Reducing Memory Usage by Disabling External Icons
**Learning:** n8n's default behavior of fetching and caching external node icons adds background network overhead and consumes precious RAM for icon storage. In highly resource-constrained environments like Render's 512MB free tier, this can contribute to OOM crashes.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to eliminate this background overhead and reclaim memory for core workflow operations.
