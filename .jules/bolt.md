...
## 2026-04-10 - Reducing Background Overhead by Disabling External Icons
**Learning:** Disabling external icon fetching via `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` reduces background network requests and idle memory usage (approx. 10-20MB) on Render's 512MB tier.
**Action:** Always disable external icon fetching in resource-constrained environments to minimize idle resource consumption and improve startup performance.
