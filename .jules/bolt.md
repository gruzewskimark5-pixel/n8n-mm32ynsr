...
## 2026-04-20 - Reducing Memory Footprint by Disabling External Icons
**Learning:** By default, n8n attempts to fetch node icons from external sources and caches them in memory. In resource-constrained environments like Render's 512MB RAM tier, this background network activity and memory-intensive caching can contribute to OOM crashes.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to prevent external icon fetching and reduce heap usage, ensuring better stability for the core workflow engine.
