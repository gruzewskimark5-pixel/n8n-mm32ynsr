...
## 2026-04-21 - Reducing Network and Memory Overhead by Disabling External Icons
**Learning:** By default, n8n attempts to fetch and cache icons for nodes from external sources. In resource-constrained environments like Render's 512MB free tier, these background network requests and the resulting memory used for icon caching can impact stability and performance.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to prevent external icon fetching, reducing both network traffic and memory consumption.
