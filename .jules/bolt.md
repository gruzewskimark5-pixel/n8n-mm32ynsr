...
## 2026-04-12 - Optimizing Memory by Disabling External Icons
**Learning:** n8n by default attempts to fetch node icons from external URLs. This involves background network requests and memory usage for icon caching. In resource-constrained environments like Render's 512MB free tier, this can lead to increased memory pressure and slower UI responsiveness.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to prevent these background network requests and reduce the memory footprint required for icon caching.
