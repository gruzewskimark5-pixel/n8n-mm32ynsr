...
## 2026-04-26 - Reducing Idle Memory with Disabled External Icons
**Learning:** n8n's feature to fetch icons from external URLs for various service nodes can consume 10-20MB of idle memory and cause unnecessary background network requests. In highly resource-constrained environments like Render's 512MB free tier, this is a significant portion of the available RAM.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to reclaim memory and reduce network overhead, aligning with the "Lean Background" strategy.
