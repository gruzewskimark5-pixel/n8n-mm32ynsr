...
## 2026-04-22 - Reducing Resource Overhead with External Icon Disabling
**Learning:** n8n's default behavior of fetching and caching external node icons triggers background network requests and increases memory usage. In resource-constrained environments like Render's 512MB free tier, this extra overhead can contribute to instability or OOM crashes during workflow execution.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to minimize background network activity and reduce the memory footprint required for icon caching.
