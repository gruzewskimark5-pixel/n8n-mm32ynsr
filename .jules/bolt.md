...
## 2026-05-21 - Optimizing Hot Paths via Property Flattening
**Learning:** Even with modern JIT compilers, deep property lookups (e.g., `this.contracts.identity.validate`) in extremely high-frequency hot paths can still be slightly slower than direct property access. In our `Kernel.route` method, which runs in ~16ns, saving even 1ns is a measurable percentage.
**Action:** In performance-critical methods, flatten deeply nested object lookups into direct class properties during construction to minimize lookup depth and provide clearer hints to the JIT compiler.
