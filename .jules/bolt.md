...
## 2026-04-06 - Optimizing Memory by Disabling External Icons
**Learning:** n8n attempts to fetch node icons from external URLs by default. In resource-constrained environments like Render's 512MB free tier, these background network requests and the subsequent memory caching of icons can increase the risk of OOM crashes and UI latency.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to prevent external icon fetching and reduce memory overhead by using built-in fallback icons.
