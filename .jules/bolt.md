...
## 2026-03-25 - Reducing Database I/O and Storage with Execution Data Persistence
**Learning:** For n8n on Render's free tier (1GB database limit), persisting execution data for successful workflows is a significant source of database bloat and I/O overhead. This can lead to rapid storage exhaustion and increased latency during workflow completions.
**Action:** Set 'EXECUTIONS_DATA_SAVE_ON_SUCCESS: none' in 'render.yaml' to only persist data for failed executions, ensuring the database remains lean and performant for critical troubleshooting data.
