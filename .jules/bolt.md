...
## 2026-04-16 - Optimizing Memory and Network with Disabled External Icons
**Learning:** n8n's default behavior of fetching and caching external node icons adds unnecessary background network requests and memory consumption. In resource-constrained environments like Render's 512MB free tier, this caching can contribute to OOM risks.
**Action:** Set `N8N_ICONS_CAN_USE_EXTERNAL: "false"` in `render.yaml` to eliminate this overhead and improve stability.
