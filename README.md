...
To prevent broken workflows from wasting resources, n8n is configured to automatically deactivate any workflow that fails 3 times in a row (`N8N_WORKFLOW_AUTODEACTIVATION_ENABLED: "true"` and `N8N_WORKFLOW_AUTODEACTIVATION_MAX_LAST_EXECUTIONS: "3"`).
...
1. **Open Environment settings:** Navigate to your service's **Environment** tab in the left-hand sidebar of the [Render Dashboard](https://dashboard.render.com/).
2. **Update variable:** To change the number of allowed consecutive failures before deactivation, update `N8N_WORKFLOW_AUTODEACTIVATION_MAX_LAST_EXECUTIONS` (default is `3`). To completely turn off auto-deactivation, set `N8N_WORKFLOW_AUTODEACTIVATION_ENABLED` to `false`.
3. **Save:** Click **Save Changes.**
4. **✅ Verify:** Once the service restarts, confirm that workflows adhere to the new failure threshold or no longer show the "Automatically deactivated" status in n8n.
