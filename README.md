...
- ⚡ **Free Tier Optimized:** Pre-tuned for Render's free tier:
  ...
  - **Lean Background:** Optimized for stability and speed by disabling non-essential features and background tasks:
    ...
    - **Operational Efficiency:**
      ...
      - 🛑 [Auto-deactivation](#workflow-automatically-deactivated) of failing workflows.
...
Follow these steps to set up your primary n8n account:

1. **Find URL:** Locate your service URL at the top of the service page in the Render Dashboard (e.g., `https://n8n-service-q975.onrender.com`).
2. **Visit URL:** Open your service URL in a new browser tab.
   > [!TIP]
   > 💤 If your service has spun down due to inactivity, it may take 1-2 minutes to "cold start." If you see a `503 Service Unavailable` error, wait a minute and refresh the page.
3. **Create account:** Follow the on-screen instructions to create your n8n owner account.
4. **✅ Verify:** Confirm you can see the n8n workflow canvas and the left-hand sidebar.
...
2. **Open Environment settings:** Navigate to your service's **Environment** tab in the left-hand sidebar of the Render Dashboard.
...
1. **Open Environment settings:** Navigate to your service's **Environment** tab in the left-hand sidebar of the Render Dashboard.
...
5. **✅ Verify:** Once the change is saved, trigger a **production** execution of the workflow (e.g., via a manual trigger or webhook) and confirm it appears in the **Executions** tab in the left-hand sidebar. Note that manual "test" runs from the editor are not saved by default.
