...
1. **Open Environment settings:** Navigate to your service's **Environment** tab in the left-hand sidebar of the [Render Dashboard](https://dashboard.render.com/).
2. **Update variable:**
   - To change the consecutive failure threshold (default is `3`), update `N8N_WORKFLOW_AUTODEACTIVATION_MAX_LAST_EXECUTIONS` to your preferred number.
   - To completely disable autodeactivation, change `N8N_WORKFLOW_AUTODEACTIVATION_ENABLED` to `false`.
3. **Save:** Click **Save Changes.**
4. **✅ Verify:** Once the service restarts, confirm that workflows that fail repeatedly no longer show the "Automatically deactivated" status in n8n (or only deactivate after reaching your custom execution threshold).
