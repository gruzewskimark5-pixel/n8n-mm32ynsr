...
4. **Verify:** Open the n8n editor and confirm the **Templates** tab appears in the left-hand sidebar.

### 📦 Missing "Community Nodes"
To save memory on Render's free tier, community nodes are disabled by default (`N8N_COMMUNITY_PACKAGES_ENABLED: "false"`). To re-enable them:
1. Navigate to the **Environment** tab in the left-hand sidebar.
2. Change `N8N_COMMUNITY_PACKAGES_ENABLED` to `true`.
3. **Save Changes**.
4. **Verify:** Open the n8n editor, click on **Settings** (gear icon) in the bottom-left corner, and confirm the **Community Nodes** option appears in the menu.
