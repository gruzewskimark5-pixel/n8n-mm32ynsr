...
To change the threshold or disable this feature:
1. **Open Environment settings:** Navigate to your service's **Environment** tab in the left-hand sidebar of the [Render Dashboard](https://dashboard.render.com/).
2. **Update variable:** Change `N8N_WORKFLOW_AUTODEACTIVATION_ENABLED` to `false` to disable auto-deactivation, or update `N8N_WORKFLOW_AUTODEACTIVATION_MAX_LAST_EXECUTIONS` (e.g., to `5`) to adjust the consecutive failure threshold.
3. **Save:** Click **Save Changes.**
4. **✅ Verify:** Once the service restarts, confirm that workflows adhere to your new failure threshold or auto-deactivation behavior in n8n.
