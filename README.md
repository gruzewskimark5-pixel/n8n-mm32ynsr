...
1. **Open Environment settings:** Navigate to your service's **Environment** tab in the left-hand sidebar of the [Render Dashboard](https://dashboard.render.com/).
2. **Update variable:** To change how many consecutive failures trigger deactivation, adjust `N8N_WORKFLOW_AUTODEACTIVATION_MAX_LAST_EXECUTIONS` (e.g., set to `5`). To disable automatic deactivation entirely, change `N8N_WORKFLOW_AUTODEACTIVATION_ENABLED` to `false`.
3. **Save:** Click **Save Changes.**
4. **✅ Verify:** Once the service restarts, confirm that workflows that fail repeatedly no longer show the "Automatically deactivated" status in n8n (or respect the updated threshold).
